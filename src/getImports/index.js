...
export function getImports(fileName, text, language) {
  ...
  setTimeout(async () => {
    ...
    try {
      if (language !== YAML) {
        await getJSImports(fileName, text, language, emitter);
      }
      else {
        await getIaCIssues(fileName, text, language, emitter);
      }
    } catch (e) {
      console.log(e);
...
async function getJSImports(fileName, text, language, emitter) {
...
async function getIaCIssues(fileName, text, language, emitter){
  await testIaC(fileName, text, emitter);
}
